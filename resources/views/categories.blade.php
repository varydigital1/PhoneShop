...
<head>
    ...
    <title>{{ config('app.name', 'PhoneShop') }} – Categories</title>
...
<body class="min-h-screen bg-gray-50">
    ...
    <header class="bg-white shadow-sm border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <nav class="flex gap-4 text-sm font-medium text-gray-600">
                ...
                <a href="/categories" class="text-orange-600 font-semibold">Categories</a>
            </nav>
        </div>
    </header>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-semibold text-gray-600 uppercase tracking-wider">Item Name</th>
                        <th class="px-4 py-3 text-left font-semibold text-gray-600 uppercase tracking-wider">Category Name</th>
                ...
                <tbody id="category-tbody" class="divide-y divide-gray-100 bg-white">
                    <!-- Rows injected by JS -->
                </tbody>
            </table>
        </div>
    ...
    (function () {
        ...
        function fetchCategories(page, search, perPage) {
            currentPage   = page;
            ...
            fetch(`/api/categories?${params}`)
                ...
                .catch(function (err) {
                    showError(err.message || 'Failed to load categories. Please try again.');
                })
                .finally(function () {
                    showLoading(false);
                });
        ...
        function renderTable(categories) {
            ...
            categories.forEach(function (c, i) {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-4 py-3 font-medium text-gray-900">${escHtml(c.item_name)}</td>
                    <td class="px-4 py-3 text-gray-700">${escHtml(c.cate_name)}</td>
                    <td class="px-4 py-3 text-gray-500">${escHtml(c.description || '—')}</td>
                    <td class="px-4 py-3">
                        <span class="inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${statusClass}">${escHtml(c.status)}</span>
                    </td>
                `;
        ...
        function makePageBtn(label, page, disabled, active) {
            ...
            if (active) {
                ...
                btn.addEventListener('click', function () {
                    fetchCategories(page, currentSearch, parseInt(perPageSel.value));
        ...
        function triggerSearch() {
            ...
            fetchCategories(1, q, parseInt(perPageSel.value));
        }
        ...
        clearBtn.addEventListener('click', function () {
            ...
            fetchCategories(1, '', parseInt(perPageSel.value));
        });

        perPageSel.addEventListener('change', function () {
            fetchCategories(1, currentSearch, parseInt(perPageSel.value));
        });
